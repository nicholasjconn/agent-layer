...
/**
 * Push an approval divergence item from a parsed command prefix.
 * @param {ApprovalItem[]} items
 * @param {string} source
 * @param {string} filePath
 * @param {string} raw
 * @param {string} prefix
 * @param {{ argv: string[] } | { reason: string }} parseResult
 * @returns {void}
 */
function pushApprovalItemFromPrefix(
  items,
  source,
  filePath,
  raw,
...
/**
 * Push an unparseable approval divergence item.
 * @param {ApprovalItem[]} items
 * @param {string} source
 * @param {string} filePath
 * @param {string} raw
 ...
 * @returns {void}
 */
function pushUnparseableApprovalItem(items, source, filePath, raw, reason) {
  items.push({
    kind: "approval",
    source,
    filePath,
...
/**
 * Collect approval divergences from a list of raw entries.
 * @param {ApprovalItem[]} items
 * @param {Set<string>} policySet
 * @param {string} source
 * @param {string} filePath
 * @param {unknown[]} entries
 * @param {(entry: unknown) => { prefix: string } | null} parseEntry
 * @returns {void}
 */
function collectApprovalEntries(
  items,
  policySet,
  source,
  filePath,
...
/**
 * Collect approval divergences from VS Code auto-approve settings.
 * @param {ApprovalItem[]} items
 * @param {string} filePath
 * @param {Record<string, unknown>} autoApprove
 * @param {Set<string>} generatedKeys
 * @returns {void}
 */
function collectVscodeApprovalEntries(
  items,
  filePath,
  autoApprove,
  generatedKeys,
...
export function collectApprovalDivergences(parentRoot, policy, enabledAgents) {
  ...
  const approvalCollectors = [
    ...
    },
    {
      ...
      collect: () => {
        ...
        if (!isPlainObject(claude)) return;
  ...
  for (const { agent, collect } of approvalCollectors) {
    if (!shouldCheckAgent(enabledAgents, agent)) continue;
    collect();
  }
...
export function collectMcpDivergences(
  ...
  const mcpCollectors = [
    {
      ...
      collect: () => {
        const geminiPath = path.join(parentRoot, ".gemini", "settings.json");
        const gemini = readJsonRelaxed(geminiPath, null);
        if (!isPlainObject(gemini) || !isPlainObject(gemini.mcpServers)) return;
    ...
    },
    {
      ...
      collect: () => {
        ...
        const claude = readJsonRelaxed(claudePath, null);
        if (!isPlainObject(claude) || !isPlainObject(claude.mcpServers)) return;
    ...
    },
    {
      ...
      collect: () => {
        ...
        if (!fileExists(codexConfigPath)) return;
        ...
        for (const [name, lines] of parsed.sections.entries()) {
          ...
          if ("server" in parsedSection) {
            ...
            if (!expected) {
              ...
              if (parsedSection.server.url !== expected.url)
                shouldInclude = true;
              if (
  ...
  for (const { agent, collect } of mcpCollectors) {
    if (!shouldCheckAgent(enabledAgents, agent)) continue;
    collect();
  }
